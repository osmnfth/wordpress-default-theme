<?php
/**
 * Δοκιμαστικό περιεχόμενο για το theme «Κοσμητεία».
 *
 * Εκτελείται από το provision.sh με:  wp eval-file /provision/seed.php
 *
 * Το περιεχόμενο είναι αντλημένο από τον ιστότοπο της Κοσμητείας της Σχολής
 * Επιστημών Υγείας του Δ.Π.Θ. (health.duth.gr): Τμήματα, όργανα, ανακοινώσεις,
 * μεταπτυχιακά και στοιχεία επικοινωνίας.
 *
 * Δημιουργεί: εικόνες, όρους ταξινομιών, 3 Τμήματα, 6 Ανακοινώσεις,
 * 6 Μεταπτυχιακά, 4 σελίδες, μενού (EL/EN), λογότυπο και τα αγγλικά
 * template parts (header-en, footer-en, home-*-en) από τα patterns.
 *
 * Ξαναχτίσιμο:  KOSMITEIA_RESEED=1 docker compose up provision
 *
 * @package Kosmiteia
 */
...
define( 'KOSM_ASSETS', '/provision/assets' );
define( 'KOSM_DEMO_VERSION', '2.0.0' );

$force = '1' === getenv( 'KOSMITEIA_RESEED' );

if ( get_option( 'kosmiteia_demo_version' ) === KOSM_DEMO_VERSION && ! $force ) {
	WP_CLI::log( '  Το δοκιμαστικό περιεχόμενο υπάρχει ήδη (KOSMITEIA_RESEED=1 για ξαναχτίσιμο).' );
	return;
}
...
function kosm_image( $filename, $title, $alt ) {
	...
	if ( ! file_exists( $path ) ) {
		WP_CLI::warning( "Λείπει η εικόνα $filename" );
		return 0;
	}
	...
	if ( ! empty( $upload['error'] ) ) {
		WP_CLI::warning( 'Αποτυχία μεταφόρτωσης ' . $filename . ': ' . $upload['error'] );
		return 0;
	}
...
function kosm_template_part( $slug, $title, $area, $content ) {
	...
	if ( $existing ) {
		...
		if ( $seeded && md5( $existing[0]->post_content ) !== $seeded ) {
			WP_CLI::log( sprintf( '    Το part «%s» έχει τροποποιηθεί - παραλείπεται.', $slug ) );
	...
	if ( is_wp_error( $part_id ) ) {
		WP_CLI::warning( $part_id->get_error_message() );
		return 0;
	}
...
function kosm_pattern( $file, $locale = null ) {
	...
	if ( ! file_exists( $path ) ) {
		WP_CLI::warning( "Λείπει το pattern $file" );
		return '';
	}
...
WP_CLI::log( '  Ρυθμίσεις ιστότοπου: ok' );
...
WP_CLI::log( '  Εικόνες: ' . count( array_filter( $images ) ) );
...
WP_CLI::log( '  Τμήματα: ' . count( $school_ids ) );
...
WP_CLI::log( '  Ανακοινώσεις: ' . $announcement_count );
...
WP_CLI::log( '  Μεταπτυχιακά: ' . $program_count );
...
$access .= kosm_map( 40.8640, 25.8040, 'Πανεπιστημιούπολη Δραγάνας', 'Κτίριο Διοίκησης, 1ος όροφος - 6ο χλμ. Αλεξανδρούπολης - Μάκρης, Τ.Κ. 68100' );
...
$contact .= kosm_map( 40.8640, 25.8040, 'Κοσμητεία Σχολής Επιστημών Υγείας', '6ο χλμ. Αλεξανδρούπολης - Μάκρης, Δραγάνα, Τ.Κ. 68100' );
...
WP_CLI::log( '  Μενού: 2 (main, main-en)' );
...
WP_CLI::log( '  Template parts: header, footer, home-hero, home-schools + 6 αγγλικά' );
...
WP_CLI::success( 'Το δοκιμαστικό περιεχόμενο δημιουργήθηκε.' );
