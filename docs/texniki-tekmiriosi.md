...
| Το `wp kosmiteia seed --force` αντικαθιστούσε λογότυπο, μενού, εικόνες εξωφύλλου και περιεχόμενο | διορθώθηκε: το seed **δημιουργεί μόνο ό,τι λείπει**. Κάθε άρθρο που φτιάχνει αποκτά `_kosmiteia_seeded_hash`· αν το περιεχόμενο έχει αλλάξει (ή δεν το έφτιαξε το seed) παραλείπεται. Μενού, λογότυπο και επιλεγμένες εικόνες δεν αντικαθίστανται ποτέ |
